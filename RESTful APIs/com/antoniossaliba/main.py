...
def get_random_cafe():
    ...
    random_number = rnd.randint(1, total)
    required_entry = cursor.execute("SELECT * FROM cafe WHERE id = ?", [random_number])
    returned_json = {}
    for entry in required_entry:
        returned_json["id"]             = entry[0]
        returned_json["name"]           = entry[1]
        returned_json["map_url"]        = entry[2]
        returned_json["img_url"]        = entry[3]
        returned_json["location"]       = entry[4]
        returned_json["seats"]          = entry[5]
        returned_json["has_toilet"]     = entry[6]
        returned_json["has_wifi"]       = entry[7]
        returned_json["has_sockets"]    = entry[8]
        returned_json["can_take_calls"] = entry[9]
        returned_json["coffee_price"]   = entry[10]
        break
